...
class Sundry
{
    ...
    public static function qrcode(
        ...
        return \xy_jx\Utils\bin\QRcode::png(
            $text,
            $filename,
            $level,
            $size,
     ...
     * @param  int  $margin  边框
     */
    public static function base64Qrcode(
        string $text,
        string $level = 'L',
        int $size = 4,
        int $margin = 1
    ...
    public static function luckDraw(
        ...
        foreach ($array as $val) {
            ...
            if ($randNum <= $val[$chance]
                && ( ! $stock
                    || $val[$stock] > 0)
            ) { //如果这个随机数小于等于数组中的一个元素 并且库存足够，则返回中将数据
                $return = $val;
                break;
            } else {
                $sum -= $val[$chance];
     ...
     * @return array
     */
    public static function arrayColumns(
        array $array,
        array $keys = [],
        $index_key = null
    ): array {
        ...
        if ($current > 32 || $result > 32) {
            ...
            if ($current > $result) { // 62进制数转换成十进制数
                ...
                for ($i = 0; $i < $len; $i++) {
                    ...
                    $dec = bcadd(
                        bcmul(bcpow($current, $len - $i - 1), $pos),
                        $dec
                    );
                }
